...
contract TreeItems_0925 is ERC1155, VRFConsumerBase{
    ...
    function getRandomNumber() public returns (bytes32 requestId) {
        ...
        require(randomResult_chk[msg.sender].chk == false,"generating");
        require(randomResult_chk[msg.sender].doubleCheck == 0," unused RandNum");
    ...
    next_Event_Nft_Id += 1;
     ...
     uint public now_Event_Reward_Id;

     struct event_Info_Of_User {
         uint isEventDonation;
         uint event_Balance;
         uint isGetNFT;
     }
       
    mapping(address => mapping (uint => event_Info_Of_User)) public eventId_to_balance;
    mapping(uint => uint) public vault_Of_Event;
  
 
    function set_EventId(uint _eventId, uint _rewardId) public onlyAdmin {
        now_EventId = _eventId;
        now_Event_Reward_Id = _rewardId;
    }
    
    function event_Validator() public {
        eventId_to_balance[msg.sender][now_EventId].isEventDonation = 1;
    }
    
    
     function event_Donate(string memory _donator) public payable {
       donate(_donator);
       require (eventId_to_balance[msg.sender][now_EventId].isGetNFT == 0 && eventId_to_balance[msg.sender][now_EventId].event_Balance >= 0 ); 
          _safeTransferFrom(owner, msg.sender, now_EventId, 1, "");
          eventId_to_balance[msg.sender][now_EventId].isGetNFT = 1 ;
