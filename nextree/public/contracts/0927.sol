...
contract TreeItems_0925 is ERC1155, VRFConsumerBase{
    ...
    uint public next_Art_Id = 20;
    
    uint public next_Event_Nft_Id = 8;
    uint public Event_Reward = 8;
    ...
    function prize_Assign(uint prize_Id) public onlyAdmin  { 
        require( prize_Id >= 20, "NFT art starts from 20");
        prize_Box.push(prize_Id);
        _safeTransferFrom(msg.sender, owner, prize_Id, 1, "");
    }
    
        function ownArtList (address _who) public view returns (uint256[] memory) {
   ...
   function event_Nft_Mint(uint amount) public onlyAdmin  { 
    require( item_ids.length < 20, "");   
    item_ids.push(next_Event_Nft_Id);
    _mint(msg.sender, next_Event_Nft_Id, amount, "");
    next_Event_Nft_Id += 1;
   }
 
    function set_Event(uint _rewardId) public onlyAdmin {
